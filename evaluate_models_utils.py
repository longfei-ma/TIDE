...
def evaluate_model_link_prediction(model_name: str, data_embedding, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
    ...
    if model_name in ['DyRep', 'TGAT', 'TGN', 'CAWN', 'TCL', 'GraphMixer', 'DyGFormer', 'FreeDyG']:
        ...
        data_embedding.set_neighbor_sampler(neighbor_sampler)
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            if model_name in ['TGAT', 'CAWN', 'TCL']:
                ...
                batch_src_node_embeddings, batch_dst_node_embeddings = \
                    data_embedding.compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids,
                ...
                batch_neg_src_node_embeddings, batch_neg_dst_node_embeddings = \
                    data_embedding.compute_src_dst_node_temporal_embeddings(src_node_ids=batch_neg_src_node_ids,
                ...
                # get positive probabilities, shape (batch_size, )
                positive_probabilities = model(graph_embeddings).squeeze(dim=-1).sigmoid()
                ...
                graph_embeddings = \
                    data_embedding.compute_src_dst_node_temporal_embeddings(src_node_ids=batch_neg_src_node_ids,
                        ...
                        node_interact_times=batch_node_interact_times,
                        num_neighbors=num_neighbors)
                # get negative probabilities, shape (batch_size, )
                negative_probabilities = model(graph_embeddings).squeeze(dim=-1).sigmoid()
...
def evaluate_model_node_classification(model_name: str, data_embedding, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            predicts = model(graph_embeddings)
...
def get_model_node_pred(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            preds.extend(predicts.detach().cpu().tolist())

            gt.extend(batch_labels.tolist())
